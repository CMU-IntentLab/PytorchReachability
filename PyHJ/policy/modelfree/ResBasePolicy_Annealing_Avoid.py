...
class ResBasePolicy_Annealing_Avoid(ABC, nn.Module):
    def __init__(
        self,
...
def _nstep_return_approximated_avoid_Bellman_equation(
    ...
    # learning a residual critic:
    # Q(s,a) = l(s) + res(s,a)
    # res(s,a) + l(s) = \gamma min(l(s), max_a' Q(s',a')) + (1-gamma)*l(s) 
    # res(s,a) = \gamma min(l(s), max_a' Q(s',a')) - gamma l(s) 
    # res(s,a) = \gamma min(0, l(s') - l(s) + max_a' res(s',a'))
    # res(s,a) = \gamma min(0, r(s,a, s') + + max_a' res(s',a'))

    target_q = gamma * np.minimum(
        np.zeros_like(rew[indices]).reshape(bsz, 1),
        rew[indices].reshape(bsz, 1) + terminal_value
    )
