...
from PyHJ.policy.modelfree.ddpg_avoid_classical_residual import avoid_DDPGPolicy_annealing_residual as DDPGPolicy

import warnings
class avoid_SACPolicy_annealing_residual(DDPGPolicy):
    ...
    def _target_q(self, buffer: ReplayBuffer, indices: np.ndarray) -> torch.Tensor:
        ...
        critic1_output = self.critic1_old(batch.obs_next, act_)
        critic2_output = self.critic2_old(batch.obs_next, act_)
    ...
    def learn(self, batch: Batch, **kwargs: Any) -> Dict[str, float]:
        ...
        if not self.warmup:
            for _ in range(self.actor_gradient_steps):
                ...
                act = obs_result1.act
                critic1_output = self.critic1(batch.obs, act)
                critic2_output = self.critic2(batch.obs, act)
                # Extract residual component (first column) for ResCritic
                if critic1_output.shape[1] == 2:
                    current_q1a = critic1_output[:, 0].flatten()
                    current_q2a = critic2_output[:, 0].flatten()
                else:
                    current_q1a = critic1_output.flatten()
                    current_q2a = critic2_output.flatten()
                pure_critic_value1 = torch.min(current_q1a, current_q2a)
                l_fn_value = torch.tensor(self.l_fn(batch.obs)).to(pure_critic_value1.device)
                #import ipdb; ipdb.set_trace()
                # check pure_critic_value1 and self.l_fn(batch.obs) shapes
                actor_loss = (
                    self._alpha * obs_result1.log_prob.flatten() -
                    (pure_critic_value1 + l_fn_value)
                ).mean()
            ...
            if self._is_auto_alpha:
                ...
                self._alpha = self._log_alpha.detach().exp()
            actor_loss = -pure_critic_value1.mean()
        else:
        ...
        result = {
            "loss/actor1": actor_loss.item(),#-pure_critic_value1.mean().item(),
