...
class Dubins_Env(gym.Env):
    ...
    def step(self, action):
        ...
        rew_next = ((self.state[0]-self.constraint[0])**2 + (self.state[1]-self.constraint[1])**2 - self.constraint[2]**2)
        ...
        info = {'rew_next': rew_next, 'rew': rew}
...
class Res_Dubins_Env(gym.Env):
    ...
    def step(self, action):
        # action is in -1, 1. Scale by self.u_max
        rew_cur = self.l_fn(self.state)# ((self.state[0]-self.constraint[0])**2 + (self.state[1]-self.constraint[1])**2 - self.constraint[2]**2)   
        ...
        self.state[2] = np.sin(theta_next)
        self.state[3] = np.cos(theta_next)
        rew_next = self.l_fn(self.state) # ((self.state[0]-self.constraint[0])**2 + (self.state[1]-self.constraint[1])**2 - self.constraint[2]**2)
        # l(x) = (x-x0)^2 + (y-y0)^2 - r^2
        ...
        for i in range(len(thetas)):
            ...
            for ii in range(nx):
                for jj in range(ny):
                    ...
                    V[ii,jj] = evaluate_V( tmp_point, policy, critic)
                    V[ii,jj] = V[ii,jj] + self.l_fn(tmp_point)
